...
def exec_timed(code, context, timeout_secs):
    ...
    t = threading.Timer( timeout_secs, thread.interrupt_main )
...
def safe_eval(code, context = {}, timeout_secs = 5):
    ...
    if not timeout_secs: timeout_secs = 5

    # XXX: context is assumed to be trusted
    ...
    if checker.walk(ast):
        return exec_timed(code, context, timeout_secs)
    else:
